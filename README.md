...
The original goal of this package was to extend/modifie some word alignment and error calculations to better understand the sorts of errors that wav2vec2 is making. This has resulted in alignments that consider word similarities and tries to create the best possible, human-like alignment between two word streams. 

## Phoebe's TODO

- [x] fix bug for word sub/deletion so we don't have a character sub with the space 
- [x] test with insertions not just deletions 
- [x] test with multiple compound errors in a sentence
- [ ] move development/debugging testing into unittest files -- IN PROGRESS
- [x] documentation
- [x] debug weridness around compound_miss_pairs (i think it's fixed?)
- [x] experiment with dynamic subcost > 1. See [below](#greater-than-1-dynamic-substitution-cost)
- [x] move code/package to it's own repo
...
In both the results object as `final_print` as well as printed to a file if the `print_to_file` option is specified in `compute_mistakes` you can find the alignments generated by the code for each sentence. A word and character level alignment looks like this
